...
- **NO CODE in Refine** — `pre_tool_use` hook blocks Write/Edit on non-spec paths.
- **Escalate** on shared triggers (ambiguity, low confidence, risk > 8, security sensitivity, destructive ops, budget) and on architect-specific triggers (multiple viable architectures without a clear winner, novel infrastructure required).
