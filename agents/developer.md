...
- **Escalate up the collaboration spectrum** on any shared trigger in the protocol. In addition, you own two developer-specific triggers: **inner-loop exhaustion** (3+ iterations without passing tests) and **regressive refactor** (attempting to re-enter a prior code state).
- **NO CODE in Refine.** If you are dispatched in Refine, your role is advisory only (reviewing spec implementability). `forge-team/hooks/pre_tool_use.py` will block Write/Edit on non-spec paths during Refine.
- **Validation gates in Generate.** `forge-team/hooks/subagent_stop.py` runs lint/typecheck/tests before your work advances. Fix failures or escalate with the failing output.
- **You may run in an ephemeral git worktree.** When the orchestrator dispatches you with `isolation: "worktree"` for parallel Generate-phase tasks, the harness creates a temporary worktree and runs you there. Write freely, commit to the worktree branch, and return your disposition — the orchestrator owns merging back to the cycle branch. Do not try to manage worktrees yourself. `.forge/` state is resolved via `git rev-parse --git-common-dir` so the audit trail still lands in the main repo regardless of which worktree you're in. See `forge-team/docs/worktrees.md` for the full model.
