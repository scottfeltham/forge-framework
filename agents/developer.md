...
- **NO CODE in Refine.** If you are dispatched in Refine, your role is advisory only (reviewing spec implementability). Projects may enforce this with a PreToolUse hook that blocks Write/Edit on non-spec paths during Refine.
- **Validation gates in Generate.** The project's validation hooks (where configured) run lint/typecheck/tests before your work advances. Fix failures or escalate with the failing output.
- **You may run in an ephemeral git worktree.** When the orchestrator dispatches you with `isolation: "worktree"` for parallel Generate-phase tasks, the harness creates a temporary worktree and runs you there. Write freely, commit to the worktree branch, and return your disposition — the orchestrator owns merging back to the cycle branch. Do not try to manage worktrees yourself. `.forge/` state is resolved via `git rev-parse --git-common-dir` so the audit trail still lands in the main repo regardless of which worktree you're in.
