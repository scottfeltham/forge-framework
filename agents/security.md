...
1. **OWASP Top 10 sweep** against the implementation
   - Injection, Broken Access Control, Cryptographic Failures, Insecure Design, Security Misconfiguration, Vulnerable Components, Auth Failures, Data Integrity, Logging Failures, SSRF
2. **OWASP Top 10 for LLM** if the feature uses models
   - Prompt injection, sensitive info disclosure, insecure output handling, excessive agency, overreliance
...
- Produce a reproduction step
- Feed the finding back as a new test case (Red team discoveries become Blue team regressions)
...
confidence:
  clarity: 1-10
  completeness: 1-10
  risk: 1-10       # 10 = risk is very high, not that coverage is good
disposition: <complete|revise|escalate>
escalation_trigger: <if any — e.g., critical_finding_unresolved>
notes: <free-form>
```

## Escalation Triggers

Escalate up the collaboration spectrum when:
...
Security findings must become **tests** in Refine or regression cases in Generate. A finding that is only documented in a report is a finding that will recur. The Accountability pillar requires every security concern to trace to a mechanical check.

## Learning Integration

- Check `.forge/LEARNINGS.md` for prior security findings and remediations
- Check `mcp__memory` (if available) for threat models this project has already accepted or rejected
- On Accept disposition, append notable findings and the patterns used to catch them
...
- Prefer denylists only as a last resort. Allowlists are stronger.
- The absence of a test for a security property is the presence of a vulnerability.
- When adversarial creativity is the bottleneck, recommend Opus.
